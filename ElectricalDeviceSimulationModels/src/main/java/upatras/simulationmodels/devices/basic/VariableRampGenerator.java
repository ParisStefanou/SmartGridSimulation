 ...
 * @author Paris
 */
public class VariableRampGenerator extends VariableRampDevice {

	boolean motor=false;

	public VariableRampGenerator(ComplexPower intial_power, ComplexPower max_power, ComplexPower ramp_rate_ms, PowerEnviroment enviroment) {
		super(intial_power, max_power, ramp_rate_ms, enviroment);
	}

	public void changeToMotor(){
		motor=true;
	}

	public void changeToGenerator(){
		motor=false;
	}
	...
	public void simulate(TimeStep simulation_step) {
		...
		if(motor)
			handlePower(new ComplexPower(average_active_power, average_reactive_power));
		else
			handlePower(new ComplexPower(-average_active_power, -average_reactive_power));
