 ...
 * @author Paris
 */
public class CoalPowerPlantTest {

	public CoalPowerPlantTest() {
	}
	...
	public void testSomeMethod() {
		...
		CoalPowerPlant ramp_load = new CoalPowerPlant(new ComplexPower(), new ComplexPower(1000000, 1000000), env);
		...
		//t=2h10m
		assertNotEquals(1000000, ramp_load.getAveragePowerProduced().getRealAsDouble(), 1);

		dsi.advanceDuration(Duration.standardHours(6));

		//t=8h10m
		assertEquals(1000000, ramp_load.getAveragePowerProduced().getRealAsDouble(), 1);

		ramp_load.decreasePower(starttime.plusHours(8).plusMinutes(10), ramp_load.getCurrentPower());
